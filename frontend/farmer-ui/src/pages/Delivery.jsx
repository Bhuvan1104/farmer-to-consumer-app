...
function Delivery() {
  ...
  useEffect(() => {
    if (role !== "farmer") {
      setError("This page is designed for farmers to plan delivery from confirmed orders.");
  ...
  const loadDeliveryData = async () => {
    try {
      ...
      setError("Failed to load delivery planning data.");
    } finally {
      setOrdersLoading(false);
    }
  };
  ...
  const saveWarehouseProfile = async () => {
    ...
    try {
      ...
      setError("Failed to save warehouse location.");
    } finally {
    ...
    <div className="delivery-page">
      <div className="delivery-container">
        <div className="delivery-header">
          ...
          <p>Choose live orders, auto-use saved customer coordinates when available, and plan single or multi-stop delivery from your warehouse.</p>
        </div>
        ...
        <div className="delivery-grid">
          <div className="delivery-card dispatch-card">
            ...
            <h2 className="section-title">Order Selection</h2>
            {ordersLoading ? <p className="helper-text">Loading active farmer orders...</p> : farmerOrders.length === 0 ? <p className="helper-text">No confirmed or in-progress orders are ready for dispatch yet.</p> : (
            ...
            <div className="action-stack">
              <button className="btn-primary full-width" onClick={calculateSingleDelivery} disabled={loading || !primaryOrder || !farmerLocation.trim()}>{loading ? "Calculating..." : "Calculate Single Delivery"}</button>
              <button className="btn-outline full-width" onClick={calculateBatchDelivery} disabled={loading || selectedOrders.length < 2 || !farmerLocation.trim()}>{loading ? "Calculating..." : "Plan Batch Route"}</button>
            </div>
          </div>
